        ...
        <>  
            <div className="bg-white flex justify-around md:justify-evenly py-3 w-full fixed top-0 left-0 z-30 shadow-lg">
                ...
                <div className="items-center gap-16 hidden md:flex md:px-20">
                    {user ? (
                        <>
                            <span className="text-[#808080] text-sm">Welcome, {user.email}</span>
                            <button 
                                onClick={handleLogout}
                                className="text-red-500 wrap-normal py-2 px-5 text-sm border-1 border-solid border-red-500 rounded-3xl cursor-pointer hover:bg-red-500 hover:text-white duration-200 ease-in-out"
                            >
                                Logout
                            </button>
                        </>
            ...
            <div className={`sm:hidden ${menuOpen ? "flex" : "hidden"} 
                ...
                {user ? (
                    <>
                        <span className="text-[#808080] text-sm mb-5">Welcome, {user.email}</span>
                        <button 
                            onClick={handleLogout}
                            className="text-red-500 py-2 px-5 text-sm border-1 border-solid border-red-500 rounded-3xl cursor-pointer hover:bg-red-500 hover:text-white duration-200 ease-in-out mb-5"
                        >
                            Logout
                        </button>
                    </>
